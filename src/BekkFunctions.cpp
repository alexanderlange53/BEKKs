...
bool valid_asymm_bekk(arma::mat& C,arma::mat& A, arma::mat& B ,arma::mat& G){
  int n =C.n_cols;
  arma::mat prod = kron(A,A)+kron(B,B)+kron(G,G);
...
double loglike_asymm_bekk(const arma::vec& theta, const arma::mat& r) {
  ...
  // check constraints
  if (valid_asymm_bekk(C, A, B , G) == false) {
    return -1e25;
  }
  //transform r to r star for asymmetry handling
  arma::mat r_star = (-1)*r;
  r_star=r_star.clean(0);
  r_star=(-1)*r_star;
  // compute inital H
  arma::mat H = (r.t() * r) / r.n_rows;
  ...
  double llv = arma::as_scalar(log(arma::det(H)) + r.row(0) * inv_gen(H) * r.row(0).t());
  for (int i = 1; i < NoOBs; i++) {
    H = CC + At * r.row(i - 1).t() * r.row(i - 1) * A + Bt * r_star.row(i - 1).t() * r_star.row(i - 1) * B + Gt * H * G;
    llv += arma::as_scalar(log(arma::det(H)) + r.row(i) * inv_gen(H) * r.row(i).t());
  }
...
//transform r to r star for asymmetry handling
  ...
  for (int i = 1; i < r.n_rows; i++) {
    ...
    dHda = 2 * D_duplication * D_gen_inv * arma::kron(arma::eye(N, N), a.t() * r.row(i-1).t() * r.row(i-1)) + GGt * dHda;
    dHdb = 2 * D_duplication * D_gen_inv * arma::kron(arma::eye(N, N), b.t() * r_star.row(i-1).t() * r_star.row(i-1)) + GGt * dHdb;
    ...
    ht = c_full + a.t() * r.row(i-1).t() * r.row(i-1) * a +b.t() * r_star.row(i-1).t() * r_star.row(i-1) * b+ g.t() * ht * g;
...
Rcpp::List  bhh_asymm_bekk(arma::mat& r, const arma::mat& theta, int& max_iter, double& crit) {
  ...
  int exit_loop = 0;
  arma::vec lik_all(max_iter+1, arma::fill::zeros);
  lik_all(0) = loglike_asymm_bekk(theta, r);
  ...
  while (count_loop < max_iter && exit_loop == 0) {
    ...
    arma::mat score_function = score_asymm_bekk(theta_loop, r);
    arma::mat outer_score = score_function.t() * score_function;

    arma::mat outer_score_inv = inv_gen(outer_score);
    arma::mat score_function_sum = arma::sum(score_function);

    double lik = loglike_asymm_bekk(theta_loop, r);
    ...
    int exit_inner = 0;
    while (j >= 0 && exit_inner == 0) {
      likelihood_candidates(j) = loglike_asymm_bekk(theta_temp.col(j), r);
      //if (likelihood_candidates(j+1) > likelihood_candidates(j)) {
      //  exit_inner = 1;
      //}
      j -= 1;
  ...
  double likelihood_final = loglike_asymm_bekk(theta_candidate, r);
  arma::mat score_final = score_asymm_bekk(theta_candidate, r);
  arma::mat s1_temp = arma::diagmat(inv_gen(score_final.t() * score_final));
  arma::mat s1 = arma::sqrt(s1_temp.diag());
...
Rcpp::List random_grid_search_BEKK(arma::mat r, int seed, int nc) {
  ...
  while(l<30000 && x < 30){
    ...
    for (int j=0; j < (n*(n+1)/2);j++){
      ...
      if(j == counter){
        theta[j]=  theta_mu[j]+arma::randn()*0.08*(1/x);
        counter+=diagonal_elements;
        diagonal_elements--;
      }
      else{
    ...
    for (int j=(n*(n+1)/2); j < numb_of_vars;j++){
      if(j == (n*(n+1)/2+diagonal_counter*(n+1)) && j< ((n*(n+1)/2) +n*n)){
        diagonal_counter++;
        theta[j]= arma::randn()*0.08*(1/x)+theta_mu[j];
      }
      else if(j == (n*(n+1)/2+n*n+(diagonal_counter-n)*(n+1)) && j>=(n*(n+1)/2+n*n)){
        diagonal_counter++;
        theta[j]= arma::randn()*0.08*(1/x)+theta_mu[j];
        ...
        theta[j]=arma::randn()*0.04*(1/x)+theta_mu[j];
      }
    }
